...
#include "xor/xorshift128plus_prng.h"  //Xorshift128+ prototype
...
/* XORSHIFT-128+ PRNG Structure */
nwipe_prng_t nwipe_xorshift128plus_prng = { "Xorshift-128+",
 ...
 * nwipe_xorshift128plus_prng_init
 *
 * Initializes the xorshift1024* PRNG state.
 *
 * NWIPE_PRNG_INIT_SIGNATURE should be defined in your NWipe codebase.
 */
int nwipe_xorshift128plus_prng_init( NWIPE_PRNG_INIT_SIGNATURE )
{
    ...
    if( *state == NULL )
    {
        /* First time initialization: allocate space for the state. */
        *state = malloc( sizeof( xorshift128plus_state_t ) );
        if( *state == NULL )
            return -1;  // Allocation failure
    }
    /* Initialize the state using the seed's data.
       The seed->s pointer is cast to uint64_t* and the number
       of 64-bit words is computed from seed->length. */
    xorshift128plus_init(
        (xorshift128plus_state_t*) *state, (uint64_t*) ( seed->s ), seed->length / sizeof( uint64_t ) );
 ...
 * nwipe_xorshift128plus_prng_read
 *
 * Fills 'buffer' with 'count' bytes of pseudorandom data generated by the
 * xorshift128+ PRNG.
 *
 * NWIPE_PRNG_READ_SIGNATURE should be defined in your NWipe codebase.
 */
int nwipe_xorshift128plus_prng_read( NWIPE_PRNG_READ_SIGNATURE )
{
    /* Pointer to current output position */
    u8* restrict bufpos = buffer;
    /* Calculate the number of complete 16-byte blocks to generate */
    size_t blocks = count / SIZE_OF_XORSHIFT128PLUS_PRNG;

    /* Fill the buffer in 16-byte blocks directly from the PRNG */
    for( size_t ii = 0; ii < blocks; ++ii )
    {
        xorshift128plus_genrand_uint128_to_buf( (xorshift128plus_state_t*) *state, bufpos );
        bufpos += SIZE_OF_XORSHIFT128PLUS_PRNG;  // Move to the next block
    }

    /* Handle remaining bytes if count is not a multiple of 128 */
    const size_t remain = count % SIZE_OF_XORSHIFT128PLUS_PRNG;
    if( remain > 0 )
    {
        unsigned char temp_output[SIZE_OF_XORSHIFT128PLUS_PRNG];
        xorshift128plus_genrand_uint128_to_buf( (xorshift128plus_state_t*) *state, temp_output );
