 ...
 */
static size_t
nwipe_effective_io_blocksize(const nwipe_context_t *c)
{
 ...
 */
static void *
nwipe_alloc_io_buffer(const nwipe_context_t *c, size_t size, int clear, const char *label)
{
    ...
    if (rc != 0 || ptr == NULL) {
        nwipe_log(NWIPE_LOG_FATAL,
                  ...
                  __FUNCTION__, label, size, align, rc);
        return NULL;
    }
...
int nwipe_random_verify( nwipe_context_t* c )
{
    ...
    while( z > 0 )
    {
        ...
        if( r != (int)blocksize )
        {
            ...
            nwipe_log( NWIPE_LOG_WARNING,
                       "%s: Partial read from '%s', %i bytes short.",
                       __FUNCTION__, c->device_name, s );
            ...
            if( offset == (off64_t) -1 )
            {
                ...
                nwipe_log( NWIPE_LOG_ERROR,
                           "Unable to bump the '%s' file offset after a partial read.",
                           c->device_name );
                free( b );
                free( d );
                return -1;
...
int nwipe_random_pass( NWIPE_METHOD_SIGNATURE )
{
    ...
    while( z > 0 )
    {
        ...
        if( r != (int)blocksize )
        {
            ...
            nwipe_log( NWIPE_LOG_WARNING,
                       "Partial write on '%s', %i bytes short.",
                       c->device_name, s );
            ...
            if( offset == (off64_t) -1 )
            {
                nwipe_perror( errno, __FUNCTION__, "lseek" );
                nwipe_log( NWIPE_LOG_ERROR,
                           "Unable to bump the '%s' file offset after a partial write.",
                           c->device_name );
                if( c->bytes_erased < ( c->device_size - z ) )
                {
                    c->bytes_erased = c->device_size - z;
        ...
        if( syncRate > 0 )
        {
            ...
            if( i >= syncRate )
            {
                ...
                if( r != 0 )
                {
                    nwipe_perror( errno, __FUNCTION__, "fdatasync" );
                    nwipe_log( NWIPE_LOG_WARNING, "Buffer flush failure on '%s'.", c->device_name );
                    nwipe_log( NWIPE_LOG_WARNING, "Wrote %llu bytes on '%s'.",
...
int nwipe_static_verify( NWIPE_METHOD_SIGNATURE, nwipe_pattern_t* pattern )
{
    ...
    if( pattern->length <= 0 )
    {
        nwipe_log( NWIPE_LOG_SANITY,
                   "nwipe_static_verify: The pattern length member is %i.",
                   pattern->length );
        return -1;
    }
     ...
     */
    d = (char *)nwipe_alloc_io_buffer(c,
                                      io_blocksize + pattern->length * 2,
                                      0,
                                      "static_verify pattern buffer");
    if( !d )
    {
        free( b );
        return -1;
    ...
    {
        nwipe_log( NWIPE_LOG_SANITY,
                   "nwipe_static_verify: lseek() returned a bogus offset on '%s'.",
                   c->device_name );
        free( b );
        free( d );
        return -1;
    ...
    while( z > 0 )
    {
        ...
        else
        {
            ...
            nwipe_log( NWIPE_LOG_WARNING,
                       "Partial read on '%s', %i bytes short.",
                       c->device_name, s );
...
int nwipe_static_pass( NWIPE_METHOD_SIGNATURE, nwipe_pattern_t* pattern )
{
    ...
    {
        nwipe_log( NWIPE_LOG_SANITY,
                   "__FUNCTION__: The pattern length member is %i.",
                   pattern->length );
        return -1;
    }
    ...
    {
        nwipe_log( NWIPE_LOG_SANITY,
                   "__FUNCTION__: lseek() returned a bogus offset on '%s'.",
                   c->device_name );
        free( b );
        return -1;
    }
        ...
        if( syncRate > 0 )
        {
            ...
            if( i >= syncRate )
            {
                ...
                if( r != 0 )
                {
                    nwipe_perror( errno, __FUNCTION__, "fdatasync" );
                    nwipe_log( NWIPE_LOG_WARNING,
                               "Buffer flush failure on '%s'.", c->device_name );
                    nwipe_log( NWIPE_LOG_WARNING,
                               "Wrote %llu bytes on '%s'.",
                               c->pass_done, c->device_name );
                    c->fsyncdata_errors++;
                    free( b );
                    if( c->bytes_erased < ( c->device_size - z ) )
